...
if [[ "$FORCE_DEFAULT" == "yes" ]]; then
    ...
    sudo qemu-img create -f qcow2 "${vm_qcow2_disk_path}" "${default_qcow2_disk_gib}G" >/dev/null
...
if [[ "$ATTACH_CONSOLE" == "yes" ]]; then
    ...
    echo "ℹ️  The VM will reboot once or twice during the re-imaging process."
