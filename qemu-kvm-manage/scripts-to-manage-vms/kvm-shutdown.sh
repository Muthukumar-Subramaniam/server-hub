...
fn_show_help() {
    cat <<EOF
Usage: kvm-shutdown [hostname]

Arguments:
  hostname  Name of the VM to graceful shutdown (optional, will prompt if not given)
EOF
}
...
# Proceed with Stop
sudo virsh shutdown "${qemu_kvm_hostname}" 2>/dev/null

echo -e "✅ VM \"$qemu_kvm_hostname\" is shutdown successfully. \n"
